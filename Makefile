...
coverage: ## check code coverage quickly with the default Python
	coverage run --source ecs_scaling_scheduler -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/ecs_scaling_scheduler.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ ecs_scaling_scheduler
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
conform	: ## Conform to a standard of coding syntax
	isort --profile black ecs_scaling_scheduler
	black ecs_scaling_scheduler tests
	find ecs_scaling_scheduler -name "*.json" -type f  -exec sed -i '1s/^\xEF\xBB\xBF//' {} +
